...
export function Projects() {
  return (
    <section id="projects" className="py-24 px-4 md:px-12 bg-[#050505]">
          ...
          {PROJECTS.map((project, idx) => (
             <CyberCard key={idx} borderColor="cyber-cyan" title={project.type.toUpperCase() + '_PRJ'} delay={idx * 150} className="flex flex-col h-full">
                <div className="flex justify-between items-start mb-4">
                   <h3 className="text-2xl font-black text-white hover:text-cyber-cyan transition-colors cursor-pointer">{project.name}</h3>
                   <span className="font-mono text-[10px] text-[#666]">{project.duration}</span>
                </div>
                ...
                <ul className="font-mono text-sm text-[#aaa] space-y-2 mb-6 flex-1">
                ...
                <div className="flex flex-wrap gap-2 mt-auto pt-4 border-t border-[#222]">
                   {project.tech.map(t => (
                      <span key={t} className="text-[10px] font-mono font-bold uppercase tracking-widest text-[#555] hover:text-cyber-cyan transition-colors cursor-default">
                         #{t}
                      </span>
                   ))}
                </div>
